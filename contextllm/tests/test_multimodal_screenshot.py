...
LOG_FILE = LOG_DIR / f'multimodal_history_{SESSION_START}.json'

def save_conversation_log(turn, text, screenshot_path, analysis):
...
print("=" * 70)
print("🎥 멀티모달 분석 (음성 + 스크린샷)")
print("=" * 70)
print("💡 팁: 말할 때 자동으로 스크린샷을 캡처하여 함께 분석합니다")
...
while True:
    try:
        ...
        # 스크린샷 캡처
        print("\n   📸 스크린샷 캡처 중...")
        screenshot_path = multimodal_analyzer.capture_screenshot(
            save_path=str(SCREENSHOT_DIR / f"screenshot_{SESSION_START}_{turn:03d}.png")
        )
        
        if screenshot_path:
            print(f"      ✅ 스크린샷 저장: {screenshot_path}")
        else:
            print("      ⚠️  스크린샷 캡처 실패 (음성만으로 분석)")
        
        # 멀티모달 분석 (음성 + 스크린샷)
        print("\n   🤖 멀티모달 분석 중 (음성 + 이미지)...")
        if screenshot_path:
            analysis = multimodal_analyzer.analyze_with_image(text, screenshot_path)
        else:
            # 스크린샷 실패 시 음성만 분석 (폴백)
        ...
        # 💾 기록 저장
        save_conversation_log(turn, text, screenshot_path, analysis)
